...
tcp_server_name1 = "OpEn/singlerate/python_cartpole" + \
                "/cartpole"
mng1 = og.tcp.OptimizerTcpManager(tcp_server_name1.replace(".", "_"), port=8333)
tcp_server_name2 = "OpEn/singlerate/python_cartpole_lifted" + \
                "/cartpole_lifted"
...
# print RMS values of state norms
rms_1 = np.sqrt(np.mean(np.square(state_norms_1)))
rms_2 = np.sqrt(np.mean(np.square(state_norms_2)))
print("RMS of state norms for NMPC: ", rms_1)
print("RMS of state norms for Lifted NMPC: ", rms_2)
...
print("RMS of control inputs for NMPC: ", rms_u_1)
print("RMS of control inputs for Lifted NMPC: ", rms_u_2)

# print average solve time
avg_solve_time_1 = np.mean(solve_time_1)
avg_solve_time_2 = np.mean(solve_time_2)
print("Average solve time for NMPC: ", avg_solve_time_1)
print("Average solve time for Lifted NMPC: ", avg_solve_time_2)
...
plt.xlim(left=0, right=right_limit)  # Set both left and right limits for the x-axis
plt.tight_layout()
#tikzplotlib.save("icp_solve_time_pm_cut-off.tex", standalone=True)
